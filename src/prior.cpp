...
arma::vec dprior_(arma::vec pvec, arma::vec dists,
  ...
  {
    if ( std::isnan(p1(i)) || std::isnan(dists(i))) {
      ...
      out[i] = R::dunif(pvec[i], p1[i], p2[i], islog[i]);
       ...
       // double tmp =  almost_equal(pvec(i), p1(i), 2) ? 1 : 1e-10;
       // out[i] = islog(i) ? log(tmp) : tmp;
      out[i] = 0;
...
NumericVector dprior(NumericVector pvec, List prior) {
  ...
  for (size_t i = 0; i < npar; i++) {
    ...
    if ( distName == 0) {
      ...
      //double tmp =  (pvec[i] - p1) < 1e-3 ? 1 : 1e-10;
...
List RestorePrior(List pprior, arma::vec p1, arma::vec p2, arma::vec lower,
  ...
  for(size_t i = 0; i < npar; i++) {
    ...
    L0 = pprior[pnames[i]]; // first parameter
    if (std::isnan(p1(i))) { // if p1 says NA, extract from the set-up
      L1 = List::create(
        _["p1"] = L0[0],
        _["p2"] = L0[1],
        _["lower"] = L0[2],
    ...
    } else {               // if not NA, extract from (input) hyper level
      L1 = List::create(
        _["p1"] = p1(i),
        _["p2"] = p2(i),
        _["lower"] = lower(i),
...
// [[Rcpp::export]]
double sumloghprior(arma::vec loc, arma::vec sca, arma::vec ldists,
  ...
  double location_den = sumlogprior(loc, ldists, lp1, lp2, llower, lupper, llog);
  double scale_den    = sumlogprior(sca, sdists, sp1, sp2, slower, supper, slog);
  double out = location_den + scale_den;
  return out;
}
