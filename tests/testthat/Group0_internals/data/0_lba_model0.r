...
helper_path <- paste0(wkdir, "data/helpers.r")
save_path <- paste0(wkdir, "data_tmp/lba_data0.rda")
source(helper_path)
...
fits1 <- ggdmc::RestartSampling_subject(fits0, sub_migration_prob = 0.00, seed = 9032)
fits <- fits1

fit <- ggdmc::RebuildPosterior(fits)
