...
wkdir <- "/media/yslin/Tui/01_Projects/ggdmc_ecosystem/ggdmc/tests/testthat/Group0_gen_lba/"
cat("\nWorking directory: ", getwd(), "\n")
helper_path <- paste0(wkdir, "helpers.r")
save_path <- paste0(wkdir, "data/lba_data2.rda")
source(helper_path)
...
p_vector <- c(
    A = .75, B = 1.25,
    mean_v.conservative.false = 1.5,
    mean_v.conservative.true = 2.15,
    mean_v.liberal.false = 1.5,
    mean_v.liberal.true = 2.5,
    sd_v.true = 0.1, t0 = .15
)
...
pop_theta_input <- ggdmc::setThetaInput(nmc = nmc, nchain = pop_nchain, pnames = pop_priors@pnames, thin = thin, report_length = 200)
