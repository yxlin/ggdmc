...
wkdir <- file.path(home_dir, "Group3_lba_hyper/")
...
fits0 <- StartSampling_hyper(hyper_dmi, pop_dmis, pop_priors,
    sub_migration_prob = 0.01, thin = 2, seed = 123, is_pblocked = TRUE
)

fits1 <- RestartSampling_hyper(fits0, sub_migration_prob = 0.00, thin = 4, seed = 123)
