...
save_path <- paste0(home_dir, "Group5_gen_ddm/data/ddm_data5.rda")
...
hyper_model <- ggdmcModel::BuildModel(
    ...
    factors = list(S = c("s1", "s2"), MOTOR_SKILL = c("excellent", "good", "poor")),
...
pop_mean <- c(
    a = 1, sz = 0.25, t0.excellent = 0.05, t0.good = 0.1, t0.poor = 0.2,
    v = 2.5, z = 0.38
)


pop_scale <- c(
    a = 0.05, sz = 0.01, t0.excellent = 0.01, t0.good = 0.02, t0.poor = 0.03,
    v = .5, z = 0.01
)
pop_dist <- ggdmcPrior::BuildPrior(
    p0    = pop_mean,
    p1    = pop_scale,
    lower = c(0, 0, 0, 0, 0, -10, 0),
...
p_prior <- ggdmcPrior::BuildPrior(
    p0 = p0,
    p1 = c(10, 10, 1, 1, 1, 10, 10),
    # p1 = c(10, 10, 2, 2, 2, 10, 10), # Work but difficult
    # p1 = rep(10, model@npar), # Very difficult to escape invalid t0 values
