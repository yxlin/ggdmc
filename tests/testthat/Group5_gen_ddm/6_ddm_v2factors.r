# q(save = "no")
cat("\n\n-------------------- Generate DDM v x c(S, NOISE) model --------------------")
rm(list = ls())
pkg <- c("ddModel", "ggdmcPrior")
suppressPackageStartupMessages(tmp <- sapply(pkg, require, character.only = TRUE))

home_dir <- "/media/yslin/Tui/01_Projects/ggdmc/tests/testthat/"
save_path <- paste0(home_dir, "Group5_gen_ddm/data/ddm_data6.rda")



hyper_model <- ggdmcModel::BuildModel(
    p_map = list(
        a = "1", v = c("S", "NOISE"), z = "1", d = "1", sz = "1", sv = "1",
        t0 = "1", st0 = "1", s = "1", precision = "1"
    ),
    match_map = list(M = list(s1 = "r1", s2 = "r2")),
    factors = list(S = c("s1", "s2"), NOISE = c("low", "moderate", "high")),
...
pop_mean <- c(
    a = 1, sz = 0.25, t0 = 0.15, v.s1.high = 1.5, v.s1.low = 2.5,
    v.s1.moderate = 2.0, v.s2.high = 1.7, v.s2.low = 2.7, v.s2.moderate = 2.2,
    z = 0.38
)
pop_scale <- c(
    a = 0.05, sz = 0.01, t0 = 0.02, v.s1.high = 0.5, v.s1.low = 0.25,
    v.s1.moderate = 0.2, v.s2.high = 0.4, v.s2.low = 0.1, v.s2.moderate = 0.15,
    z = 0.01
)
...
pop_dist <- ggdmcPrior::BuildPrior(
    p0    = pop_mean,
    p1    = pop_scale,
    lower = c(0, 0, 0, -10, -10, -10, -10, -10, -10, 0),
...
dat <- simulate(sub_model, nsim = 240, parameter_vector = p_vector, n_subject = 1)
hdat <- simulate(pop_model, nsim = 240, n_subject = 32)
