...
require(testthat)
context("sim logistic")

test_that("sim logistic", {
  rm(list = ls())
  ...
  model <- BuildModel(
    ...
    constants = NULL,
    type      = "logit")
  p.vector <- c(a0 = -.55, a1 = .08, a2 = -.81, a3 = 1.35, b = .267)
  ntrial <- 10
  ...
  ppprior <- BuildPrior(
    dists = c( rep("tnorm2", 4), "gamma"),
    p1    = c(a0 = -.55, a1 = .08, a2 = -.82, a3 = 1.35, sigma = .267),
    p2    = c(a0 = .19, a1 = .30, a2 = .41, a3 = .26, sigma = .15),
    lower = rep(NA, 5),
    upper = rep(NA, 5))
  set.seed(123)
  ps1 <- rprior(ppprior)
  ...
  ntrial <- 40
  set.seed(123)
  b <- rnorm(ntrial, 0, ps1[5])
  linpred <- X %*% ps1[1:4] + b
  pi <- ggdmc:::invlogit(linpred)
  ni <- sample(1:100, ntrial, replace = TRUE)
  ri <- rbinom(ntrial, ni, pi)

  dat2 <- data.frame(S = X[,2], E = X[,3], Y = ri, N = ni)
  data.table(dat2)

  linpred <- X %*% p.vector[1:4] + p.vector[5]
  ...
  est <- summary(fit, start = 1, recover = TRUE, ps = p.vector, verbose = TRUE)
  #                   b0   b1    b2   b3       sd
